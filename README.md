...
主要功能
统一管理多台已授权服务器上的 Xray 服务。
Agent 安装时自动备份并兼容已有配置，降低迁移风险。
图形化创建 VLESS Reality、XHTTP、Shadowsocks 等服务。
可视化配置转发关系和路由规则。
支持单主机配置备份、恢复以及平台状态备份。
根据已配置服务生成常用客户端订阅格式。
Agent 心跳、配置快照和服务状态监测。
按主机、入站和客户端查看流量使用情况。
提供受控的客户端流量查看页面，和IDC厂商同步按照月固定日期重置受控客户端流量。
提供基础连通性和健康状态检测。
TG通知：主机离线通知、流量查询反馈、客户端到期通知、客户端流量警告。
